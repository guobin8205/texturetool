...
class BuildTool(object):
        ...
        try:
            ...
            if self.command == "flist":
                ...
                if not is_texturepacker_valid():
                    ...
                    return
                self.project_path = args.output
                args.work_path = self.project_path
                self.build_project()
                pass
            else:
                pass
    ...
    def convert_resource(self):
        if args.image_option != "ETC1" and args.image_option != "PVRTC4" and args.image_option != "ETC2":
            return None
        return_data = {}
        compress_data = []
        ...
        if args.poolSize > 1 and len(files) > 0:
            ...
            compress_data = pool.map(converter.convert_to_texture, files)
            pool.close()
            pool.join()
        else:
            for file in files:
        ...
        for data in compress_data:
            if data is None:
                continue
            relpath = os.path.relpath(data.get("file"), self.project_path)
            src_key = relpath.replace('.pvr.ccz', data.get("ext")).replace('\\', '/')
